...
public class ConnectionPool {
	...
	private int initialConnections = 1; // 默认连接池的初始大小

	private int incrementalConnections = 5;// 默认连接池自动增加的大小

	private int maxConnections = 20; // 默认连接池最大的大小
	...
	public ConnectionPool(String path) {
		...
		try {
			...
			this.jdbcDriver = p.getProperty("jdbc.driverClassName");
			this.dbUrl = p.getProperty("jdbc.url");
			this.dbUsername = p.getProperty("jdbc.username");
			this.dbPassword = p.getProperty("jdbc.password");
			
			String maxConnections = p.getProperty("cpool.maxPoolSize");
			this.maxConnections = Integer.parseInt(maxConnections==null?this.maxConnections+"":maxConnections);
			
			String minConnections = p.getProperty("cpool.minPoolSize");
			this.initialConnections = Integer.parseInt(minConnections==null?this.initialConnections+"":minConnections);
			
			String autoIncremental = p.getProperty("cpool.autoIncremental");
			this.incrementalConnections = Integer.parseInt(autoIncremental==null?this.incrementalConnections+"":autoIncremental);
	...
	private boolean isValid(Connection conn) {
		...
		try {
			return conn.isValid(maxConnections);
		} catch (SQLException e) {
